...
function calcularResistenciaColor() {
    ...
    const b3Element = document.getElementById('band3');
    const b3 = b3Element ? b3Element.value : "0";
    ...
    if(modoBandas === 5 && b3Element) {
        document.getElementById('v-band3').style.backgroundColor = b3Element.options[b3Element.selectedIndex].style.backgroundColor;
    }
...
function abrirHerramienta(id) {
    document.querySelectorAll('.ventana-capa').forEach(capa => capa.style.display = 'none');
    const destino = document.getElementById(id);
    if (destino) {
        destino.style.display = 'block';
        window.scrollTo(0,0);
        // Calculamos una vez al abrir para que no salga vacío
